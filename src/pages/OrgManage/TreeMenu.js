...
class TreeMenu extends PureComponent {
  ...
  initTree = async ({ selectNode = 'first', expand = 'no' }) => {
    ...
    const { orgManage: { selectedKey, orgNodes: prevOrgNodes }, dispatch } = this.props
    ...
    if (selectNode === 'first' || selectNode === 'last') {
      ...
      dispatch({ type: 'orgManage/save', payload: { selectedKey, selectedType } })
    }
    // 规则：先下后上 都无则parent
      ...
      if (children.length > 1) {
        ...
        dispatch({ type: 'orgManage/save', payload: { selectedKey: String(newNode.id), selectedType: newNode.type } })
      } else {
        dispatch({ type: 'orgManage/save', payload: { selectedKey: String(parentId), selectedType: 'ORG' } })
      }
    }
    ...
    if (expand === 'self') {
      if (!expandedKeys.includes(selectedKey)) {
        ...
        dispatch({ type: 'orgManage/save', payload: { expandedKeys: [...expandedKeys], autoExpandParent: false } })
      }
    } else if (expand === 'parent') {
      const parentId = String(orgNodes.find(item => String(item.id) === selectedKey).parentId)
  ...
  onSelect = (
    ...
    dispatch({
      type: 'orgManage/save',
      payload: { selectedKey: String(dataRef.id), selectedType: dataRef.type },
    })
  }
  ...
  render () {
    const { orgManage: { selectedKey } } = this.props
