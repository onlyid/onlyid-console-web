...
class TreeLinkDialog extends PureComponent {
  ...
  submit = async () => {
    ...
    const { onClose, type, userPool: { selectedKey: userId } } = this.props
  ...
  onSearch = keyword => {
    ...
    if (keyword) {
      const expandedKeys = orgNodes.filter(({ name }) => name.includes(keyword)).map(item => String(item.id))
